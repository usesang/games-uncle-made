...
모바일에서는 모음집이나 공룡 게임을 **홈 화면에 추가**한 뒤 아이콘으로 열면 브라우저 탭 대신 전체 화면형 웹앱으로 실행됩니다. 브라우저에서 바로 게임을 시작할 때도 지원되는 기기에서는 전체 화면을 요청합니다. 다만 주소창·상태 표시줄을 숨길 수 있는 범위는 모바일 브라우저의 정책에 따릅니다.
